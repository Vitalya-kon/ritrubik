...
function getData(
    ...
    $art = ($page * $kol) - $kol;
        ...
        $gameItemQuery = $db->query("SELECT * FROM `game` WHERE id > 0 ORDER BY CASE WHEN actual = 1 THEN 0 ELSE 1 END, CASE WHEN actual = 1 THEN releaseGame END DESC, id DESC, RAND($seed) LIMIT $kol OFFSET $art");
    ...
    function formatDate($dateString) {
        ...
        foreach ($gameItemQuery as $rows) {
            ...
            $alt = $rows["alt"];
            $name = $rows["name"];
            $releaseGame = !empty($rows['releaseGame'])  ? formatDate(date('d.m.Y', $rows['releaseGame'])) : '';
            ...
            // Разделение данных по запятой и сохранение их в массив
                ...
                foreach ($dataArray as &$value) {
                    ...
                    if ($value === 'PlayStation 3' OR $value === 'PlayStation 4' OR $value === 'PlayStation 5') {
